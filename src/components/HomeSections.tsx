...
export default function HomeSections({ categories, initialContent }: { categories: Category[]; initialContent?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const load = () => {
      fetch(`/api/site-content?_t=${Date.now()}`, { cache: 'no-store' })
        .then(r => {
          if (!r.ok) throw new Error(`HTTP ${r.status}`);
          return r.json();
        })
        .then((json: { data?: SiteContent }) => {
          if (mounted && json.data) setContent(json.data);
        })
        .catch((err: Error) => {
          console.error('HomeSections fetch failed:', err.message);
        });
    };
  ...
  const heroBg = hero.bgImage || 'https://images.unsplash.com/photo-1587836374828-4dbafa94cf0e?w=1800&q=80';
  const imageOnly = hero.imageOnly === true;
  ...
  const ctaSubtitle = content.cta?.subtitle || 'Browse our collection and place your order directly on WhatsApp in minutes.';
